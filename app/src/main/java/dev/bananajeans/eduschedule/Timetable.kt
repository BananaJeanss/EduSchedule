...
object EduPageParser {
    ...
    fun parse(raw: String, revision: Revision): Timetable {
        ...
        val periods = rows("periods").associateBy { it.optString("period", it.getString("id")) }
        ...
        val cards = rows("cards").flatMap { card ->
            ...
            val groupIds = lesson.strings("groupids")
            val classIds = (lesson.strings("classids") + groupIds.mapNotNull { groups[it]?.optString("classid") }).distinct()
            val teacherIds = lesson.strings("teacherids")
            val roomIds = card.strings("classroomids")
            ...
            val endPeriod = ((period.toIntOrNull() ?: 0) + lesson.optInt("durationperiods", 1).coerceAtLeast(1) - 1).toString()
            ...
            card.getString("days").mapIndexedNotNull { day, enabled ->
                if (enabled != '1' || day > 6) null else {
                    ...
                    val end = time(endPeriod, bell, day, "endtime")
