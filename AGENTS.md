...
## Data invariants
Select the last non-hidden revision effective on the requested date. Never automatically choose a future revision. For times, prefer day-specific overrides; otherwise accept an explicit valid range in the published period label before generic bell/base fields, because some exports contain stale structured metadata. Treat long published teaching blocks as authoritative instead of blindly multiplying editor `durationperiods`; conventional periods up to 60 minutes may be extended. Days/weeks are positional bit strings, IDs are strings (including `*` and `-`). Keep split lessons distinct in the model even when the class UI groups them into one block. Unknown times cannot generate calendar events. Each export date needs its own effective revision. Multiweek cycles require an explicit user selection until calendar mapping is supported.
